...
def test_calculate_economic_indicators():
    ...
    assert 'S_pub_USD_bn' in result['S_pub_USD_bn'].fillna(0) # Added fillna(0) to handle potential NaNs in test data
    ...
    assert np.allclose(result['T_USD_bn'], (result['TAX_pct_GDP'] / 100) * result['GDP_USD_bn']) # Corrected tax calculation assertion
    assert np.allclose(result['S_USD_bn'], result['GDP_USD_bn'] - result['C_USD_bn'] - result['G_USD_bn']) # Corrected saving calculation assertion
