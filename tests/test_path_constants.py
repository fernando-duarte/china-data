...
class TestPathConstants:
    ...
    def test_get_search_locations_relative_to_root(self):
        ...
        assert "imf_files" in locations
        
        # Check that input_files contains expected directories
        input_files = locations["input_files"]
        assert isinstance(input_files, list)
        assert "input" in input_files
        assert "." in input_files
        
        # Check that imf_files contains expected patterns
        imf_files = locations["imf_files"]
        assert isinstance(imf_files, list)
        assert any("input" in path for path in imf_files)
