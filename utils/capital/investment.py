...
def calculate_investment(capital_data: pd.DataFrame, delta: float = 0.05) -> pd.DataFrame:
    ...
    logger.info(
        f"Using {df_clean.shape[0]} years of capital stock data from "
        f"{df_clean['year'].min()} to {df_clean['year'].max()}"
    )
    ...
    try:
        ...
        if investments:
            ...
            if len(non_na) > 0:
                ...
                logger.info(
                    f"Investment range: {min_i:.2f} to {max_i:.2f} billion USD, average: {mean_i:.2f} billion USD"
                )
