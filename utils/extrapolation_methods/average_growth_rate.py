...
def extrapolate_with_average_growth_rate(
    ...
    if len(historical) < min_data_points:
        logger.info(
            f"Insufficient data for average growth rate on {col} (need {min_data_points}, have {len(historical)})"
        )
    ...
    try:
        ...
        logger.info(
            f"Applied average growth rate of {growth_percent:.2f}% to {col} using "
            f"{len(growth_rates)} historical periods"
        )
