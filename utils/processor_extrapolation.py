...
def _apply_methods(
    ...
    for col in cols:
        ...
        if col in gdp:
            df_updated, success, method = extrapolate_with_arima(df, col, yrs, min_data_points=5, order=(1, 1, 1))
            if success:
                df = df_updated
        ...
        if (col in demographic or col in human) and not success:
            df_updated, success, method = extrapolate_with_linear_regression(df, col, yrs, min_data_points=2)
            if success:
                df = df_updated
                info[col] = {"method": method, "years": yrs}
                continue
