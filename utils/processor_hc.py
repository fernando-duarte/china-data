...
def project_human_capital(processed_data: pd.DataFrame, end_year: int = 2025) -> pd.DataFrame:
    ...
    try:
        ...
        logger.info(
            f"Human capital data: {total_rows} total rows, {non_na_rows} non-NA rows ({na_percentage:.1f}% missing)"
        )
            ...
            if len(hc_data_not_na) > 0:
                ...
                for year in years_to_project:
                    if year not in hc_data["year"].values:
                        hc_data = pd.concat(
                            [hc_data, pd.DataFrame({"year": [year], "hc": [np.nan]})], ignore_index=True
                        )
        ...
        years_to_project = list(range(int(last_year_with_data) + 1, end_year + 1))
        logger.info(
            f"Will project human capital for {len(years_to_project)} years: "
            f"{min(years_to_project)} to {max(years_to_project)}"
        )
