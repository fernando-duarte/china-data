...
def validate_year_range(df: pd.DataFrame, year_column: str = "year") -> None:
    ...
    if year_column not in df.columns:
        raise DataValidationError(column=year_column, message=f"Year column '{year_column}' not found for range validation.")
    ...
    if max_year > Config.MAX_REASONABLE_YEAR:
        logger.warning(f"Data found for year {max_year}, which is after Config.MAX_REASONABLE_YEAR {Config.MAX_REASONABLE_YEAR}")
...
def validate_numeric_values(
    ...
    if series_to_validate.empty and not allow_na and not df[column].empty:
        raise DataValidationError(column=column, message=f"Column '{column}' contains only NA values but allow_na is False.")
...
INDICATOR_VALIDATION_RULES: Dict[str, Dict[str, Any]] = {
    ...
    "SP_POP_TOTL": {"strict_positive": True, "min_value": 1000}, # Population, min 1000 to be reasonable
    "SL_TLF_TOTL_IN": {"strict_positive": True, "min_value": 1000}, # Labor Force
    "BX_KLT_DINV_WD_GD_ZS": {"min_value": -100, "max_value": 200}, # FDI % GDP, wider range for safety
    
    # PWT original column names (before renaming in merged_data)
    "rgdpo": {"strict_positive": True}, 
    "rkna": {"strict_positive": True}, 
    "hc": {"min_value": 0.5, "max_value": 5.0}, # Human Capital Index
    "pl_gdpo": {"strict_positive": True}, # Price Level
    "cgdpo": {"strict_positive": True}, # Consumption GDP Output side

    # IMF data (already renamed)
    "TAX_pct_GDP": {"min_value": 0, "max_value": 100}, # Tax as % of GDP
}

def validate_dataframe_with_rules(df: pd.DataFrame, rules: Dict[str, Dict[str, Any]], year_column: str = "year") -> None:
